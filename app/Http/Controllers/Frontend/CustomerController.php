...
class CustomerController extends Controller
{
    ...
    public function checkout()
    {
        ...
        return view('frontEnd.layouts.customer.checkout', compact('shippingcharge', 'bkash_gateway','select_charge', 'shurjopay_gateway'));
    }
    public function order_save(Request $request)
    {
        ...
        'phone.regex' => 'Please enter a valid 11-digit phone number starting with 01.',
    ]);
        ...
        if ($sms_gateway) {
            ...
            $data = [
                ...
                "type" => 'text',
                "senderid" => "$sms_gateway->serderid",
                "message" => "Dear $request->name!\r\nYour order ($order->invoice_id) has been successfully placed. Track your order tracking https://ozybd.com/customer/order-track and Total Bill $order->amount\r\nThank you for using $site_setting->name"
            ];
            $ch = curl_init();
            curl_setopt($ch, CURLOPT_URL, $url);
            curl_setopt($ch, CURLOPT_POST, 1);
    ...
    {
        $order = Order::where(['id' => $request->id, 'customer_id' => Auth::guard('customer')->user()->id])->with('orderdetails', 'payment', 'shipping', 'customer','orderdetails.image')->firstOrFail();
